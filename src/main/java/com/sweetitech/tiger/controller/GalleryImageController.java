...
public class GalleryImageController {
	...
	public ResponseEntity<GalleryImage> createGalleryImage(@Valid Principal principal,
		...
		if (hasPrivilege(WRITE_PRIVILEGE, userService.findUserByEmail(principal.getName()))) {
			...
			GalleryImage galleryImageToBeAdded =  new GalleryImage(title, description, image);
			
			for (int i = 0; i < tags.length; i++) {
				Tag tempTag = tagService.findByTag(tags[i]);
				if( tempTag == null) {
					tempTag = new Tag(tags[i]);
				}
				galleryImageToBeAdded.getTag().add(tempTag);
				
				tempTag.getGalleryImages().add(galleryImageToBeAdded);
				tagService.updateTag(tempTag);
			}
			
			
			GalleryImage galleryImage = galleryImageService
					.addGalleryImage(galleryImageToBeAdded);
	...
	public ResponseEntity<GalleryImage> updateGalleryImage(@Valid Principal principal,
			...
			@RequestParam(value = "tagIds", required = true) String[] tags
			) {
		...
		if (hasPrivilege(WRITE_PRIVILEGE, userService.findUserByEmail(principal.getName()))) {
			...
			for (int i = 0; i < tags.length; i++) {
				...
				galleryImageToBeUpdated.getTag().add(tempTag);
				tempTag.getGalleryImages().add(galleryImageToBeUpdated);
				tagService.updateTag(tempTag);
			}
