	...
	public ResponseEntity<Product> createProduct(@Valid Principal principal,
		...
		if (hasPrivilege(WRITE_PRIVILEGE, userService.findUserByEmail(principal.getName()))) {
			...
			for (int i = 0; i < tags.length; i++) {
				...
				product.getPtag().add(tempPtag);
				tempPtag.getProduct().add(product);
				ptagService.updatePtag(tempPtag);
			}
...
//			}
	...
	@GetMapping("/category")
	 public @ResponseBody Page<Product> showAllTagsByCategoryId(@RequestParam(value = "groupId", required = true)  long groupId) {
		return productService.findAllProductByGroups_id(groupId, 0);
	 }
	
	@GetMapping("/category/page")
	 public @ResponseBody Page<Product> showAllTagsByCategoryIdByPage(
			 @RequestParam(value = "groupId", required = true)  long groupId, 
			 @RequestParam(value = "pageId", required = true)  int pageId) {
		return productService.findAllProductByGroups_id(groupId, pageId);
	 }
