...
//
//			Banner banner = bannerService.addBanner(new Banner(pageNumber, position, url , image));
//			
	...
	public ResponseEntity<Banner> updateBanner(@Valid Principal principal,
		...
		//	BannerSize bannerSize = bannerSizeRepository.findById(bannerSizeId);
			...
			if(pageNumber !=null && pageNumber != 0) {
				toBeUpdated.setPageNumber(pageNumber);
			}if(position !=null && position != 0) {
				toBeUpdated.setPosition(position);
			}
...
//	}
	...
	@GetMapping("/specific")
	public @ResponseBody Banner getBannerByPageNUmberAndPosition(
			@RequestParam(value = "pageNumber", required = true) Long pageNumber,
			@RequestParam(value = "position", required = true) Long position) {

		Banner banner = bannerService.findBannerByPageNumberAndPosition(pageNumber,position);
		if (banner != null) {
			return banner;
		} else {
