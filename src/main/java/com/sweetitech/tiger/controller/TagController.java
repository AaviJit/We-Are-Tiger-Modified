...
public class TagController {
	...
	public ResponseEntity<Banner> createTag(@Valid Principal principal,
			...
			@RequestParam("galleryImageId") long galleryImageId, 
	...
	@GetMapping("/")
	public @ResponseBody Page<Tag> showAllTags() {
		Page<Tag> tagList = tagService.findAll(0);
		return tagList;
	}
	
	@GetMapping("/page")
	public @ResponseBody Page<Tag> showAllTagsByPage(@RequestParam(value = "pageId", required = true) int pageId) {
		Page<Tag> tagList = tagService.findAll(pageId);
		return tagList;
	}
